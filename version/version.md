...
- **页脚加浏览量**：GitHub 链接下方显示「累计浏览量 / 当日浏览量」。用第三方计数服务 [abacus](https://abacus.jasoncameron.dev)（免注册、CORS 开放）；累计用固定 key，当日用 `d-<北京时间日期>` 作 key。只在 http(s) 下计数——`file://` 打开本地预览不计数，避免污染；服务不可用则整行隐藏。
